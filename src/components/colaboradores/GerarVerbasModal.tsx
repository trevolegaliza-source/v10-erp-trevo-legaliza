...
interface Props {
  ...
  onConfirm: (selectedIds: string[], year: number, month: number) => void;
  isPending: boolean;
}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onConfirm(Array.from(selected), year, month)} disabled={isPending || selected.size === 0}>
            {isPending ? 'Gerando...' : `Gerar Verbas (${selected.size})`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
