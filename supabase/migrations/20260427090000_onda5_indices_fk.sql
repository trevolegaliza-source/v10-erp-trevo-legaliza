-- =============================================
-- Onda 5 — Índices em FKs (audit fix #25)
-- =============================================
-- Postgres não cria índice automático em FK (só em PK e UNIQUE).
-- Sem índice na coluna FK, qualquer query do tipo
-- "SELECT ... WHERE cliente_id = $1" vira sequential scan O(N).
--
-- Esta migration é DEFENSIVA: cada índice é tentado dentro de um
-- bloco DO $$ ... EXCEPTION pra que se a coluna não existir mais
-- (renomeada/removida em migration posterior), não quebra a aplicação
-- inteira da migration. Apenas pula com NOTICE.
--
-- Tudo IF NOT EXISTS — seguro re-rodar.
-- =============================================
...
-- Limpeza do helper (uso one-shot)
