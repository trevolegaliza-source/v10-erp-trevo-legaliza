-- =============================================
-- Tabela empresas_config — suporte multi-empresa
-- =============================================
-- Até hoje dados da empresa (razão social, CNPJ, PIX, banco,
-- WhatsApp, site) viviam hardcoded na RPC get_cobranca_por_token
-- e em src/lib/mensagem-cobranca.ts. Isso:
--   (a) travava o sistema numa empresa só (Trevo Legaliza)
--   (b) exigia redeploy pra trocar banco, chave PIX, telefone, etc.
--
-- Escopo desta migration: APENAS o Financeiro.
--   - Cria tabela empresas_config 1:1 com empresa_id
--   - Seed com dados atuais da Trevo
--   - Atualiza RPC get_cobranca_por_token pra LER dessa tabela
--     (com fallback pros valores atuais se registro não existir)
--
-- Outros hardcodes (PDFs, portfolio público, login, DRE) permanecem
-- pra uma sessão futura de multi-empresa ampla.
-- =============================================
...
CREATE TABLE IF NOT EXISTS public.empresas_config (
  ...
  pix_chave      TEXT,
  pix_banco      TEXT,
  whatsapp       TEXT, -- formato internacional (ex: 5511934927001)
  site           TEXT,
  nome_fantasia  TEXT, -- opcional, pra display
...
CREATE TRIGGER trg_empresas_config_updated_at
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public._empresas_config_touch_updated_at();

-- 2) RLS
ALTER TABLE public.empresas_config ENABLE ROW LEVEL SECURITY;

-- SELECT: qualquer authenticated da empresa vê a config da própria empresa
...
-- INSERT/UPDATE/DELETE: só master da própria empresa
...
-- 3) Seed com dados atuais da Trevo pra todas as empresas já existentes
--    (Hoje sabemos que Thales tem 1 empresa. Fazer por SELECT DISTINCT
--    garante que funciona mesmo se houver mais.)
...
-- 4) Helper pra resolver config da empresa com fallback garantido
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- Fallback: valores atuais da Trevo, pra nunca quebrar UI pública
...
-- 5) Atualiza get_cobranca_por_token pra usar empresas_config da empresa da cobrança
--    Mantém assinatura idêntica — UI não precisa mudar.
--    Precisamos DROP antes porque RETURNS TABLE não deixa CREATE OR REPLACE
--    se retorno mudar (neste caso não muda, mas por segurança).
