...
-- 1) colaboradores: INSERT/UPDATE/DELETE exigem role master/financeiro
...
-- 2) Expiração obrigatória em propostas (orcamentos)
...
-- 3) Auditoria de acesso a RPCs públicas
...
DROP POLICY IF EXISTS "acessos_publicos_select_master" ON public.acessos_publicos_log;
...
-- Atualiza get_proposta_por_token pra rejeitar expirados + log
...
-- 4) handle_new_user: REMOVE fallback "LIMIT 1 master qualquer"
...
-- 5) Hash da senha master (compat backwards)
...
DROP POLICY IF EXISTS "master_pw_config_select_master" ON public.master_password_config;
CREATE POLICY "master_pw_config_select_master" ON public.master_password_config
  FOR SELECT TO authenticated
  USING (get_user_role() = 'master');

DROP POLICY IF EXISTS "master_pw_config_insert_master" ON public.master_password_config;
CREATE POLICY "master_pw_config_insert_master" ON public.master_password_config
  FOR INSERT TO authenticated
  WITH CHECK (get_user_role() = 'master');

DROP POLICY IF EXISTS "master_pw_config_update_master" ON public.master_password_config;
