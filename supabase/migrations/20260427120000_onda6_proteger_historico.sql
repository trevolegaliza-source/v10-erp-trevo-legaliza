-- =============================================
-- Onda 6 — Proteger histórico financeiro (audit fix #5)
-- =============================================
-- Hoje: ON DELETE CASCADE em vários FKs apontando pra clientes/processos.
-- Risco: 1 clique ou 1 bug deleta cliente → cascateia e apaga TODO o histórico
-- (lançamentos, cobranças, processos, documentos). Sem volta. Sem backup auto.
--
-- Aqui: troca CASCADE por RESTRICT nessas FKs críticas. Banco passa a BLOQUEAR
-- delete de cliente/processo se houver dependência. Soft delete (is_archived)
-- continua funcionando normal e PRESERVA TUDO.
--
-- Defensivo: usa pg_constraint pra detectar nome real da constraint
-- (Supabase às vezes nomeia auto: "lancamentos_cliente_id_fkey", às vezes
--  recebe nome custom). DROP CONSTRAINT IF EXISTS evita quebrar se já foi
--  alterado em ambiente intermediário.
--
...
DECLARE
  ...
  v_new_conname TEXT;
BEGIN
  -- Localiza nome real da FK (pode ser auto: <tabela>_<col>_fkey ou custom)
  ...
  -- Verifica se já é RESTRICT (confdeltype = 'r' = RESTRICT, 'a' = NO ACTION,
...
-- ===== FKs críticas: histórico financeiro =====
-- DELETE em clientes deve FALHAR se houver lançamentos/cobranças/processos.
-- Solução do usuário: arquivar (is_archived=true), nunca deletar bruto.
...
-- ===== Garante is_archived em cobrancas e orcamentos =====
-- (já existe em clientes e processos)
ALTER TABLE public.cobrancas
  ADD COLUMN IF NOT EXISTS is_archived BOOLEAN DEFAULT FALSE;
ALTER TABLE public.orcamentos
  ADD COLUMN IF NOT EXISTS is_archived BOOLEAN DEFAULT FALSE;

-- Índices parciais pra queries "só ativos" ficarem rápidas
...
-- ===== Função pública: arquivar cliente em cascata =====
-- Frontend chama isto em vez de DELETE. Marca cliente + processos relacionados
-- como arquivados em transação atômica. Cobranças/lançamentos NÃO são tocados
-- (continuam visíveis no histórico financeiro — esse é o ponto).
...
BEGIN
  ...
  -- Arquiva cliente + processos atomicamente
...
-- ===== Função pública: desarquivar cliente =====
...
-- =============================================
-- Fim Onda 6 — histórico financeiro protegido
-- =============================================
-- Pós-deploy: tentar `DELETE FROM clientes WHERE id = '...'` no SQL Editor
-- deve dar erro:
--   ERROR: update or delete on table "clientes" violates foreign key
--          constraint "lancamentos_cliente_id_fkey" on table "lancamentos"
--   DETAIL: Key (id)=(...) is still referenced from table "lancamentos".
--
-- Pra apagar mesmo (caso EXTREMO, duplicata sem histórico):
--   1. Confirmar que NÃO há lancamentos: SELECT COUNT(*) FROM lancamentos WHERE cliente_id = '...'
--   2. Confirmar cobrancas: SELECT COUNT(*) FROM cobrancas WHERE cliente_id = '...'
--   3. Confirmar processos: SELECT COUNT(*) FROM processos WHERE cliente_id = '...'
--   4. Se tudo zero: DELETE funciona. Se algum >0, é o RESTRICT te protegendo.
-- =============================================
